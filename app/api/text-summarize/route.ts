...
export async function POST(req: Request): Promise<NextResponse> {
  try {
    ...
    // Check credits
    const creditCheck = await checkCredits({ toolName: 'text-summarize' });
    ...
    await recordUsage({ toolName: 'text-summarize' });
    ...
    // Record failed usage
    try {
      await recordUsage({ toolName: 'text-summarize', success: false });
    } catch (recordError) {
