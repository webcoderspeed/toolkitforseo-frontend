...
export async function POST(req: Request): Promise<NextResponse> {
  try {
    ...
    // Check credits
    const creditCheck = await checkCredits({ toolName: 'sentence-checker' });
    ...
    await recordUsage({ toolName: 'sentence-checker' });
    ...
    // Record failed usage
    try {
      await recordUsage({ toolName: 'sentence-checker', success: false });
    } catch (recordError) {
